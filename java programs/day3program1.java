...
public class day3program1 {
    public static void main(String[] args) {
        ...
        int left = 0, area = 0, maxLen = 0;

        for (int right = 0; right < n; right++) {
            int room = roomSizes[right];
            freqMap.put(room, freqMap.getOrDefault(room, 0) + 1);
            area += room;

            // Shrink window if constraints are violated
            while (area > maxArea || freqMap.get(room) > maxFreq) {
                int leftRoom = roomSizes[left];
                freqMap.put(leftRoom, freqMap.get(leftRoom) - 1);
                if (freqMap.get(leftRoom) == 0) {
                    freqMap.remove(leftRoom);
                }
                area -= leftRoom;
                left++;
            }
...
//         int max=0;
//         for(int right=0; right<n; right++){
//             map.put(roomSizes[right], map.getOrDefault(roomSizes[right],0)+1);
//             area+=roomSizes[right];
//             if(area>maxArea){
//                 map.put(roomSizes[right], map.getOrDefault(roomSizes[right],0)-1);
//                 area-=roomSizes[right];
//                 if(isValid(map, maxFreq)){
//                     max = Math.max(max,right-left);
//                     map.put(roomSizes[left], map.getOrDefault(roomSizes[left],0)-1);
//                     area-=roomSizes[left]; 
//                     left++;
//                     right--;                 
//                 }
...
//     }
//     public static boolean isValid(HashMap<Integer,Integer> map, int maxFreq){
