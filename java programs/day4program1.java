...
public class day4program1{
    public static void main(String[] args){
        ...
        int k = sc.nextInt();
        int m = sc.nextInt();
        int[] mviRatings = new int[n];
        int[] restrictRatings = new int[m];
        for(int i=0; i<n; i++){
            mviRatings[i] = sc.nextInt();
        }
        for(int i=0; i<m; i++){
            restrictRatings[i] = sc.nextInt();
        }
        ...
        int l=0;
        for(int r=0; r<n; r++){
            set.add(mviRatings[r]);
            sum+=mviRatings[r];
            if(r-l >= k-1){
                if(set.size()==k && isValid(set, restrictRatings) ){
                    max = Math.max(max,sum); 
                }
                set.remove(mviRatings[l]);
                sum-=mviRatings[l];
                l++;
            }
        }
    ...
    public static boolean isValid(HashSet<Integer> set, int[] restrictRatings){
        for(int num : restrictRatings){
            if(set.contains(num) ){
                return false;
            }
        }
