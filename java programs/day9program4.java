// The Indian Army has established multiple military camps at strategic
// locations
// along the Line of Actual Control (LAC) in Galwan. These camps are connected
// in
// a hierarchical structure, with a main base camp acting as the root of the
// network.
...
// - If no left connection exists but a right connection does, follow the right
// connection.
// - The leftmost leaf camp is NOT included in this boundary.
// 3. The leaf camps (i.e., camps with no further connections), ordered from
// left to right.
// 4. The right boundary (in reverse order):
// - Starts from the root’s right child and follows the rightmost path
// downwards.
// - If a camp has a right connection, follow it.
// - If no right connection exists but a left connection does, follow the left
// connection.
